...
void loop() {
  ...
  case 1:  // * * * * * P U R G E  S T A T E  * * * * *
    ...
    if(initialValveClose == 0){
      ...
      if(deltaPSI<.03 && digitalRead(rPin1) == LOW && !valveRequest || millis() > Timeout){
      ...
      if(deltaPSI<.05 && digitalRead(rPin1) == HIGH && valveRequest || millis() > Timeout){      
        Timeout = millis() + 60000;//60s timeout
        if(delayInterlock == false){ //delay statements
          delayInterlock = true;
  ...
  case 2: // * * * * * B U R P   S T A T E * * * * *
    ...
    if(burpCounter < burpLoops){
      ...
      if(deltaPSI<.03 && digitalRead(rPin1) == LOW && !valveRequest || millis() > Timeout){
        ...
        if(delayInterlock == false){ //delay statements
          delayInterlock = true;
          delayTime = millis() + 5000; //set time for 5 seconds into future
        }
